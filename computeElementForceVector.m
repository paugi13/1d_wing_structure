function F_el = computeElementForceVector(n_i, Nel, x, l_e_vector, L1, L2, l, M)
...
for i = 2:n_nod     % Right side
    ...
    lambda_e = M/(4*(L1+L2)) + 3*M*(L1-x(i))/(2*L2^2);
    if x(i)>5
        q_e = l*(1-(x(i)-(L1)/L2))*(1+(x(i)-(L1)/L2));
        lambda_e = 9.81*M/(4*(L1+L2));
    end
    q_e_res = q_e - lambda_e;
    F_el(:,i-1) = q_e_res*l_e_vector(i-1,1)/2*[1; l_e_vector(i-1,1); 1;...
        -l_e_vector(i-1,1)];
end
